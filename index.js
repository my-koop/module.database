...
function processInMemoryQuery(ast) {
    ...
    switch (ast.type) {
        case "select":
            ...
            if (table) {
                switch (ast.where.operator) {
                    case "=":
                        rows = table.filter(function (row) { return row[ast.where.left.column] == ast.where.right.value; });
                        break;
                }
            }
            return rows;
        case "insert":
            ...
            var table = memoryDB[tableName];
            var newElem = ast.set.reduce(function (newElem, col) {
...
function getInMemoryConnection(callback) {
    var connection = {
        ...
        query: function (sql, values, callback) {
            ...
            try {
                ...
                logger.info(ast);
                var res = processInMemoryQuery(ast);
                res.fieldCount = 0;
